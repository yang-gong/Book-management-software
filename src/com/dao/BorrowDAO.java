...
import java.sql.*;
 public class BorrowDAO {
    ...
    public int insertBorrow(ReaderForm readerForm,BookForm bookForm,String operator){
   	...
   	ca.add(Calendar.DATE, days);// 增加的天数，可以改变的
   	d = ca.getTime();
   	String backTime = st.format(d);
           //计算归还时间
          ...
          try {
            if (rs1.next()) {
               ...
               //	String date = st.format(d);
               	...
               	Calendar ca = Calendar.getInstance();
               	ca.add(Calendar.DATE, backTime+days);// 30为增加的天数，可以改变的
               	d = ca.getTime();
               	String backTime1 = st.format(d);
               	System.out.println("增加天数以后的日期: " + backTime1);
                
                String sql = "UPDATE tb_borrow SET backtime='" + backTime1 +
                             "' where id=" + id + "";
    ...
    Date dateU = new Date();
    java.sql.Date date = new java.sql.Date(dateU.getTime());
    String sql="select r.id,borr.borrowTime,borr.backTime,book.barcode,book.bookname,r.name readername,r.barcode readerbarcode from tb_borrow borr join tb_bookinfo book on book.id=borr.bookid join tb_reader r on r.id=borr.readerid where borr.backTime <='"+date+"'";
    ResultSet rs=conn.executeQuery(sql);
    ...
    try {
        while (rs.next()) {
            ...
            form.setBorrowTime(rs.getString(2));
            form.setBackTime(rs.getString(3));
            form.setBookBarcode(rs.getString(4));
            form.setBookName(rs.getString(5));
            form.setReaderName(rs.getString(6));
            form.setReaderBarcode(rs.getString(7));
            coll.add(form);
            System.out.println("图书条形码："+rs.getString(4));
        }
    } catch (SQLException ex) {
...
return coll;
    ...
    public Collection bookBorrowSort() {
       String sql = "select * from (SELECT bookid,count(bookid) as degree FROM tb_borrow group by bookid) as borr join (select b.*,"
       		+ "c.name as bookcaseName,p.pubname,t.typename from tb_bookinfo b left join tb_bookcase c"
       		+ " on b.bookcase=c.id join tb_publishing p on b.ISBN=p.ISBN join tb_booktype t on b.typeid=t.id where b.del=0)"
